...
class ExchangeInfo(Base):
    ...
    venue_type: Mapped[int] = mapped_column(
        TINYINT,
        nullable=False,
        server_default=text("'0'"),
        comment="交易所类型：0=CEX中心化交易所, 1=DEX去中心化交易所",
    )
    display_name: Mapped[str | None] = mapped_column(String(100), comment="展示名称")
...
class ExchangeSymbol(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="fk_exchange_symbol_exchange",
        ),
    ...
    inst_type: Mapped[int] = mapped_column(
        TINYINT, primary_key=True, comment="产品类别：0:SPOT现货, 1:perpetual永续合约, 2:FUTURES交割合约, 3:OPTION期权"
    )
    base_asset: Mapped[str | None] = mapped_column(String(30), comment="基础资产")
    quote_asset: Mapped[str | None] = mapped_column(String(20), comment="计价资产")
    price_precision: Mapped[int | None] = mapped_column(TINYINT, comment="价格精度")
    quantity_precision: Mapped[int | None] = mapped_column(TINYINT, comment="数量精度")
    tick_size: Mapped[str | None] = mapped_column(String(24), comment="价格最小变动步长")
    step_size: Mapped[str | None] = mapped_column(String(24), comment="数量最小变动步长")
    status: Mapped[int | None] = mapped_column(
        TINYINT, comment="交易状态：0=ACTIVE(交易中), 1=HALTED(暂停), 2=PENDING(待上线),3 =CLOSED(已下线)"
    )
    onboard_time: Mapped[int | None] = mapped_column(BigInteger, comment="上架时间，即该交易对首次可交易的时间")
    created_at: Mapped[datetime.datetime | None] = mapped_column(
        DateTime, server_default=text("CURRENT_TIMESTAMP"), comment="创建时间"
    )
    updated_at: Mapped[datetime.datetime | None] = mapped_column(
        DateTime, server_default=text("CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"), comment="最后更新时间"
    )
